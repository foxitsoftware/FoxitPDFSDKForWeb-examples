...
//
// This file is the source code of pkcs7.exe.
//
// NOTE: before building this file, user should ensure that openssl environment has been prepared:
...
unsigned char* PKCS7Sign(const char* cert_file_path, std::string cert_file_password,
        void* plain_text,int size, unsigned long& signed_data_size) {
        PKCS7* p7 = NULL;
        EVP_PKEY* pkey = NULL;
        X509* x509 = NULL;
        ...
        unsigned char* der = reinterpret_cast<unsigned char*>(malloc((size_t)der_length));
        memset(der, 0, (size_t)der_length);
...
int main(int argc, char *argv[]) {
        ...
        if (argv[1][0] == 's') {
                ...
                */
                ...
                unsigned long signedSize = 0;
                void *signedBuffer = PKCS7Sign(argv[2], std::string(argv[3]), buffer, size, signedSize);
